...
function App() {
  ...
  return (
    <div className="relative w-full h-screen bg-[#020202] text-white overflow-hidden font-sans">
      ...
      {/* Native CSS Animations to replace framer-motion */}
      <style>{`
      ...
      <div className="relative z-10 w-full h-full pointer-events-none flex flex-col justify-between">
        ...
        <header className="p-6 md:p-10 flex justify-between items-center">
          ...
          <nav className="hidden lg:flex items-center gap-2 p-1 bg-white/5 backdrop-blur-md rounded-full border border-white/10 animate-header-nav">
            {['Simulation Introduction', 'SUMO Simulation', 'Project Architecture', 'Meet the Team'].map((tab) => (
              <button 
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-6 py-2 rounded-full text-xs font-bold tracking-widest uppercase transition-all duration-300 pointer-events-auto relative overflow-hidden group ${
                  activeTab === tab 
          ...
          <div className="w-[200px] hidden lg:block"></div> {/* Spacer to balance the header */}
        </header>

        {/* Main Content */}
        <main className="flex-1 flex items-center px-6 md:px-20 max-w-7xl mx-auto w-full">
          {activeTab === 'Simulation Introduction' && (
            <div key="intro" className="max-w-2xl animate-tab-content">
          ...
          {activeTab === 'SUMO Simulation' && (
            <div key="simulation" className="max-w-2xl animate-tab-content">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-6 backdrop-blur-md">
          ...
          {activeTab === 'Project Architecture' && (
            <div key="architecture" className="max-w-4xl w-full animate-tab-content">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-6 backdrop-blur-md">
    ...
    └── frontend/
        └── index.html        # Three.js live viewer (no build step)`}
          ...
          {activeTab === 'Meet the Team' && (
            <div key="team" className="max-w-5xl w-full animate-tab-content">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-6 backdrop-blur-md">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pointer-events-auto">
                {[
                  ...
                  <div
                    key={person.name}
                    className="group relative p-8 rounded-3xl bg-black/60 border border-white/10 backdrop-blur-md hover:bg-black/80 hover:border-[#FFD700]/50 transition-all cursor-pointer overflow-hidden animate-team-card"
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-6 relative z-10">
                      <div className="flex items-start justify-end">
                        <div className="p-2 bg-white/5 rounded-lg border border-white/10 group-hover:border-[#FFD700]/30 transition-colors duration-300">
                          <ArrowRight className="w-5 h-5 text-gray-500 group-hover:text-[#FFD700] transform group-hover:translate-x-1 transition-transform duration-300" />
                        </div>
                      </div>
                      ...
                      <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                        ...
                        <div className="flex gap-3">
                          <button 
                            ...
                            className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-[#FFD700] hover:text-black transition-colors duration-300"
                          >
